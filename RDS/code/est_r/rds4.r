...
#globalVariables(c("RID", "rc", 'variable'))
...
# New in RDS4 are additional measures to assess recruitment activity.
...
rds.set_groups_diff_rec = function(in_var, ID, recruiterID, groups=NULL)
{
    # Suppress Warnings for non-standard eval
    RID <- rc <- variable <- NULL

    if ( is.null(groups)) {
		u=as.factor(in_var)
	} else {
		u=factor(in_var, levels=sort(groups))
	}
	
	null_table = tibble(variable = levels(u))
	
	rds = tibble(variable=as.character(in_var), id=ID, RID=recruiterID)
	
    mean_rec = (rds
      %>% filter(! is.na(RID))       
      %>% group_by(RID)
      %>% summarize(rc=n())
      %>% left_join(rds, by=c('RID'='id'))
      %>% group_by(variable)
      %>% summarize(m_ra=mean(rc))
      %>% right_join(null_table, by=c('variable'))
    )
    
    #print( mean_rec )
    #print( null_table )
    
   	rec = list(group=mean_rec$variable, mean_rec=mean_rec$m_ra)
   	return(c(rec))
