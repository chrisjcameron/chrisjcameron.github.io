...
def rdsim_iter(network_names, sim_params):
    ...
    for net_name in network_names:
        ...
        for node in G.nodes():
            ...
            #assign class
            if int(G.node[node]['fresh']):     G.node[node]['class'] = '0' 
            if int(G.node[node]['sophomore']): G.node[node]['class'] = '1' 
            if int(G.node[node]['junior']):    G.node[node]['class'] = '2' 
            if int(G.node[node]['senior']):    G.node[node]['class'] = '3' 
