#RDS simulator class
import random, networkx as nx, sys #, pylab as p

#these modules act as libraries of recruitment activity
#attribute assignment
#and recruitment choice functions
import attdist, recact, reccho
...
class RDNet:
    ...
    def objNet(self,net) :
        if isinstance(net.nodes()[0], Recruit):
            ...
            objDict = {}
            for i in net.nodes() :
                rec = Recruit(i,self.attlisttemplate)
                objNet.add_node(rec)
                objDict[i] = rec
...
class RDSim :
    def __init__(self, RDNet=None, reclimit=-1, seeds=1, seedcho=random.sample, \
    rec=recact.rec_fixed, recparams = 1, rec_meth=reccho.meth_random, limit_sample_size=False, add_seeds=False ) :
        
        #network
        self.network = RDNet.network.copy()
        
        #erase self network so we don't keep 2 large objects in memory        
        RDNet.network.clear()
        
        #Copy is not deep, objects in graph are same, only structure is copied
        self.master = self.network.copy()
        
        #recruitment template (generated from attlisttree)
        self.recstrings = genreccombs(RDNet.attlisttree,[],[])
        
        #correct rectemplate:
        self.rectemplate = [(a,b) for a in self.recstrings for b in self.recstrings] 
        ...
        #seed choice function
        self.seedcho = seedcho
        
        #recruitment activity function
        self.rec = rec
        
        #recruitment activity params
        self.recparams = recparams
        
        #recruitment choice function
        ...
        #seeds
        self.seedList = self.seedcho(self.network.nodes() , self.seeds)  
    ...
    def Simulate(self, reclimit=None, replace="no", limit_sample_size=False, add_seeds=False ) :
        ...
        while self.sampleSize < reclimit and len(self.currentWave)>0:
            ...
            for node in self.currentWave:
                ...
                rec_num = self.rec(self.recparams) 
                ...
                recruit_priority = self.rec_meth( node, potentials)
            ...
            if self.add_seeds and len(self.currentWave) <= 0 and self.sampleSize < reclimit:
                ...
                if unrecruited_list:  #the list is not empty
                    ...
                    new_seed = self.seedcho( unrecruited_list , 1) 
                    new_seed=new_seed[0]
...
if __name__ == '__main__' :
    ...
    mysim = RDSim(nx.fast_gnp_random_graph(10000, .001), attdist.nothing, 0, 50, 5, random.sample, ract, 1, reccho.meth_random)
