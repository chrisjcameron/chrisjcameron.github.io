...
#import sys
import itertools as itr #, pylab as p
import functools
...
class RDNet:
    def __init__(self, network=nx.DiGraph(), attLevels = [('att1',('0','1')),('att2',('0','1'))], att_pairs=None ) :
        ...
        #network
        self.network = self.objNet(network, att_pairs)
    ...
    def objNet(self, net, att_pairs=None) :
        if isinstance(net.nodes()[0], Recruit):
            return net.copy()
        else:
            objNet = nx.Graph()
            objNet.name = net.name
            objDict = {}
            for i in net.nodes() :
                rec = Recruit(i,self.attlisttemplate, net.degree(i))
                ## Added cjc73 2013-11-20 to improve node attribute retention
                if att_pairs is not None:
                    for index, label in enumerate(att_pairs):
                        rec.attlist[index] = net.node[i][label]
                objNet.add_node(rec)
                objDict[i] = rec
    ...
    def printPop(self):
        print('sample attlist: {}'.format(self.network.nodes()[0].attlist))
    ...
    # cjc73 2009-5-20: added
    # cjc73 2013-11-20: modified to skip reID if all nodes have unique ID
    def reID(self) :
        """ Most networks should be reIDed after absorbing other nets"""
        nodes = self.nodes()
        id_list = [ x['id'] for x in self.network.node.values() ]
...
class RDSim :
    def __init__(self, RDNet=None, 
        reclimit=-1, 
        seeds=1, 
        seedcho=seedchoice.seed_random, 
        seed_choice_params=None,
        rec=recact.rec_fixed, 
        rec_params=1, 
        rec_meth=reccho.meth_random, 
        rec_meth_params=None,  
        ...
        self.att_levels = RDNet.attlisttree.copy()
        #recruitment template (generated from attlisttree)
        self.node_value_list = list(itr.product(*[x[1] for x in RDNet.attlisttree]))
        self.recstrings = [''.join(x) for x in self.node_value_list]
        # itr replaces genreccombs(RDNet.attlisttree,[],[])  
         
        #correct rectemplate:
        self.rec_value_list = list(itr.product(self.node_value_list, repeat=2))
    ...
    #Renumber node ID to eliminate duplicate ID in network
    # cjc73 2009-5-20: 
    def reID(self, preserve=False) :
        if preserve:
            nodes = sorted(self.master.nodes(), key=get_id)
    ...
    def tie_report_complex(self):
        ...
        for node in self.network.nodes():
            nodeattstr = str(functools.reduce(lambda x,y: x+y,node.attlist))
            for nbr in self.network.neighbors(node):
                nbrattstr = str(functools.reduce(lambda x,y: x+y, nbr.attlist))
    ...
    def pop_report_complex(self):
        ...
        for node in self.network.nodes():
            ...
            nodeattstr = functools.reduce(lambda x,y: x+y,node.attlist)
            #print nodeattstr
